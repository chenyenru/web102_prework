...
function addGamesToPage(games) {
    ...
    for (let i = 0; i < games.length; i++) {
        ...
        <h3>${games[i].name}</h3>
        <caption>${games[i].description}</caption>
        <br /> <br />
...
const total_contributions = GAMES_JSON.reduce(
    (accumulator, currentValue) => accumulator + currentValue.backers, 0
)
...
const total_raised = GAMES_JSON.reduce(
    (accumulator, currentValue) => accumulator + currentValue.pledged, 0
)
...
function filterUnfundedOnly() {
    ...
    let toShow = GAMES_JSON.filter((game) => {
        return game.pledged < game.goal;
    })
    // use the function we previously created to add the unfunded games to the DOM
    addGamesToPage(toShow);
}
...
const displayStr = 
`A total number of \$${total_contributions.toLocaleString("en-US")} has been raised for ${GAMES_JSON.length} games, yet ${unfundedGames.toLocaleString("en-US")} remain unfunded. We need your help to fund these amazing games!`;

// create a new DOM element containing the template string and append it to the description container
const descriptions = document.createElement("p");
descriptions.innerHTML = displayStr;
descriptionContainer.append(descriptions);
